...
app.get('/api/persons/:id', (request, response, next) => {
  Person
    ...
    .then((person) => {
      if (person) { response.json(person.toJSON()); } else { response.status(404).end() ;}
    })
    .catch(error => next(error));
});
...
app.put('/api/persons/:id', (request, response, next) => {
  const receivedPerson = request.body;
  if (!receivedPerson.name) { return response.status(400).json({ error: 'Name is missing' }); }
  if (!receivedPerson.number) { return response.status(400).json({ error: 'Number is missing' }); }
  let updatedPerson = {
    name: receivedPerson.name,
    number: receivedPerson.number,
  };
